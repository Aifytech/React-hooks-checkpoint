...
const MovieCard = ({ movie }) => {
  const { id, title, description, posterURL, rating, trailer } = movie;
  ...
  return (
    <Link to={`/trailer?id=${id}`} className="group block">
      <div className="group relative bg-movie-surface rounded-lg overflow-hidden transition-all duration-300 hover:bg-movie-surface-hover hover:scale-105 hover:shadow-[var(--shadow-card-hover)] shadow-[var(--shadow-card)]">
        <div className="relative overflow-hidden">
          <img
            src={posterSrc}
            alt={title}
            className="w-[300px] h-[150px] object-cover rounded-lg transition-transform duration-300 group-hover:scale-110"
            onError={(e) => {
              e.currentTarget.src =
